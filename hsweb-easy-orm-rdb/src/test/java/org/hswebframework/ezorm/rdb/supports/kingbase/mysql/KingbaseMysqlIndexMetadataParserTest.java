...
public class KingbaseMysqlIndexMetadataParserTest {

    @Test
    public void test() {
        SyncSqlExecutor sqlExecutor = new TestSyncSqlExecutor(new KingbaseMysqlConnectionProvider());
        try {
            KingbaseMysqlSchemaMetadata schema = new KingbaseMysqlSchemaMetadata(KingbaseMysqlConnectionProvider.getSchema());
            KingbaseMysqlIndexMetadataParser parser = new KingbaseMysqlIndexMetadataParser(schema);
