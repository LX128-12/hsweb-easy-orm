...
public class KingbaseOracleQueryAdvancedTest extends AbstractKingbaseOracleJdbcTestSupport {
    ...
    public void testOrderByAscAndDesc() {
        ...
        try {
            ...
            Assert.assertEquals("Bob", ascResult.get(1).get("name"));
            Assert.assertEquals("Charlie", ascResult.get(2).get("name"));

            List<Map<String, Object>> descResult = operator.dml()
                .query(table)
                .orderBy(SortOrder.desc("score"), SortOrder.asc("name"))
                .fetch(mapList())
                .sync();
            Assert.assertEquals(95, ((Number) descResult.get(0).get("score")).intValue());
        } finally {
