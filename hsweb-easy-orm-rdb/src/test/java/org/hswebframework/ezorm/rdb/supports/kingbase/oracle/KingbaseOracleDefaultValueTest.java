...
public class KingbaseOracleDefaultValueTest extends AbstractKingbaseOracleJdbcTestSupport {

    @Test
    public void testDefaultValueSysdate() {
        String table = randomName("test_def_sysdate_");
        String qualified = currentSchema() + "." + table;
        execute("create table " + qualified
            + " (id int primary key, name varchar(64), created_at timestamp default sysdate)");
        try {
            ...
            Map<String, Object> row = operator.dml()
                ...
                .fetch(singleMap())
                .sync();
            Assert.assertEquals("with_default", row.get("name"));
