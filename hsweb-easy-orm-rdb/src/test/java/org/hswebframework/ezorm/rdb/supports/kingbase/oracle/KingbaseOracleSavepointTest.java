...
public class KingbaseOracleSavepointTest extends AbstractKingbaseOracleJdbcTestSupport {
    ...
    @Test
    public void testNestedSavepoint() throws Exception {
        String table = randomName("test_nested_sp_");
        String qualified = currentSchema() + "." + table;
        execute("create table " + qualified + " (id int primary key, name varchar(64))");
        try {
            try (Connection conn = provider.getConnection()) {
                conn.setAutoCommit(false);
                conn.createStatement().execute("insert into " + qualified + " values (1, 'level0')");

                Savepoint sp1 = conn.setSavepoint("sp1");
                conn.createStatement().execute("insert into " + qualified + " values (2, 'level1')");

                Savepoint sp2 = conn.setSavepoint("sp2");
                conn.createStatement().execute("insert into " + qualified + " values (3, 'level2')");
                conn.rollback(sp2);
